...
describe("renderMermaidDiagrams", () => {
  ...
  it("should initialize mermaid on first use", async () => {
    // Reset module to clear mermaidInitialized state
    vi.resetModules();
    const { renderMermaidDiagrams } = await import("../../src/render/mermaid");
    ...
    // Simulate mermaid loading from CDN after a delay
    vi.useFakeTimers();
    const renderPromise = renderMermaidDiagrams(container);

    // Advance time to allow mermaid to be set
    setTimeout(() => {
      (window as any).mermaid = mockMermaid;
    }, 10);
    await vi.advanceTimersByTimeAsync(10);

    // Advance time to allow polling to find mermaid (polling checks every 50ms)
